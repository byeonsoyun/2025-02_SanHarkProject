...
6. **일정 추가 요청 감지:** 
   사용자가 "일정 추가", "캘린더에 추가", "일정 등록", "일정 넣어줘" 등의 요청을 하면 
   ...
   - "12월 25일 크리스마스 파티" → [EVENT_ADD]{"title": "크리스마스 파티", "start_date": "2025-12-25", "end_date": "2025-12-25", "description": "크리스마스 파티"}[/EVENT_ADD]

7. **답변 스타일:** 
   - 질문에 대한 핵심 정보를 먼저 명확하게 제시
   - 일시, 장소, 대상 등 구체적 정보는 항목별로 정리
   - HTML 태그(</br>, <strong> 등)를 절대 사용하지 말 것
   - 마지막에 정보 출처를 명시 (예: "이 정보는 충북대학교 학사일정/공지사항/기숙사 안내를 통해 확인된 내용입니다.")
8. **답변 형식:**
...
class RAGService:
    ...
    def retrieve_context(self, query: str) -> List[Dict]:
        ...
        try:
            ...
            # 식단 관련 질문인지 확인 (더 명확하게)
            is_menu_query = any(keyword in query for keyword in ['식단', '메뉴', '오늘 밥', '이번주 식단'])
            
            final_results = []
            menu_results = []  # 식단 관련 결과 우선 저장
            
            # 쿼리셋을 리스트로 변환하여 순회 (top K개만 먼저 가져옴)
            for item in results_queryset[:self.TOP_K * 3]: # 일단 TOP_K의 3배를 가져와서 필터링
                item_dict = {
...
답변 끝에 관련 정보의 제목과 URL을 **간결한 목록 형태**로 반드시 명시해 주세요.
